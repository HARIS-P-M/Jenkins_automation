  ...
  return (
    <>
      ...
      <header className="fixed top-4 left-4 right-4 z-[100] flex justify-center">
        <div className="w-full max-w-4xl glass-card rounded-[1.5rem] px-4 py-2 flex justify-between items-center shadow-2xl border-white/5">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-gradient-to-br from-violet-500 to-pink-500 rounded-xl flex items-center justify-center shadow-lg shadow-violet-500/20">
            ...
            <span className="font-bold tracking-tight text-sm hidden sm:block">Nexus <span className="gradient-text">Contacts</span></span>
          </div>
          ...
          <div className="flex items-center gap-2">
            <div className="relative">
              <button 
                ...
                className={`p-2.5 rounded-xl transition-all ${showTools ? 'bg-violet-500/20 text-violet-400' : 'bg-white/5 hover:bg-white/10 text-text-muted hover:text-white'}`}
              >
              ...
              {showTools && (
                <div className="absolute top-full right-0 mt-3 w-56 glass-card rounded-2xl p-2 shadow-2xl border-white/10 fade-in">
            ...
            <button 
              onClick={toggleTheme}
              className="p-2.5 rounded-xl bg-white/5 hover:bg-white/10 transition-colors text-text-muted hover:text-white"
            >
              {theme === 'dark' ? (
            ...
            <button 
              ...
              className="flex items-center gap-2 pl-2 pr-1.5 py-1.5 bg-white/5 hover:bg-white/10 rounded-xl transition-all border border-white/5 group"
            >
              <div className="w-7 h-7 bg-violet-500 rounded-lg flex items-center justify-center text-[10px] font-bold shadow-lg shadow-violet-500/20 group-hover:scale-105 transition-transform">
      ...
      <nav className="fixed bottom-6 left-1/2 -translate-x-1/2 z-[100] w-[90%] max-w-lg">
        <div className="glass-card rounded-[2rem] p-2 flex justify-around items-center shadow-[0_20px_50px_rgba(0,0,0,0.5)] border-white/10">
          {navItems.map(({ key, label, icon, badge }) => {
            ...
            return (
              <button
                ...
                className={`flex flex-col items-center justify-center p-3 rounded-2xl relative transition-all duration-300 ${isActive ? 'bg-violet-500/10 scale-110' : 'hover:bg-white/5'}`}
              >
                {badge > 0 && (
                  <span className="absolute top-2 right-2 bg-pink-500 text-white text-[9px] font-black rounded-full min-w-[16px] h-[16px] flex items-center justify-center px-1 shadow-lg border border-[#050505]">
                    {badge > 99 ? '99+' : badge}
                  </span>
                )}
                <NavIcon name={icon} active={isActive} />
                <span className={`text-[9px] mt-1 font-bold tracking-wide uppercase transition-colors duration-300 ${isActive ? 'text-violet-400' : 'text-text-muted'}`}>
                  {label}
                </span>
                {isActive && (
                  <div className="absolute -bottom-1 w-1 h-1 bg-violet-400 rounded-full blur-[1px]" />
                )}
              </button>
            )
          })}
